...
public class MessagePublisher {
	...
	public void sendOrderMessage(Order order) {
	    ...
	    kafkaTemplate.send(KafkaTopicConfig.TOPIC_ORDER, order);
	}
	
	public void sendOrderMessageWithCallback(Order order) {
		//String orderJson = mapper.writeValueAsString(order);
	     CompletableFuture<SendResult<String, Order>> future = kafkaTemplate.send(KafkaTopicConfig.TOPIC_ORDER, order);
